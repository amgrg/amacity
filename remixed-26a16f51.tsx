...
import { Search, Plus, Store, LogOut, User, Package, BarChart3, Settings, Loader2 } from 'lucide-react';
...
const ProductSearchApp = () => {
  ...
  // Ottimizzazione: memorizza tutti i prodotti
  ...
  // API simulate ottimizzata con cache
  const searchProducts = useCallback((term) => {
    ...
    // Controlla cache
    if (searchCache[lowerTerm]) {
      return searchCache[lowerTerm];
    }
    
    // Ricerca ottimizzata - massimo 8 risultati
    const results = allProducts
      .filter(product => product.searchText.includes(lowerTerm))
      .slice(0, 8); // Limita a 8 risultati per performance
  ...
  // Debounced search con indicatore di caricamento
  const handleSearch = useCallback((term) => {
    ...
    // Debounce di 150ms per ricerca fluida
              ...
              {searchResults.length > 0 ? (
                <>
                  <div className="px-4 py-2 text-xs text-gray-500 border-b border-gray-100">
                    ...
                    {searchResults.length === 8 ? ' (primi 8)' : ''}
                  </div>
